...
const AttractionsPage = () => {
  ...
  const handleCardClick = (id) => {
    // Navigate to the AttractionDetailsPage and pass the id via the URL
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {filteredLocations.length > 0 ? (
            filteredLocations.map(location => (
              <div
                key={location._id}
                className="border rounded-xl overflow-hidden shadow-xl transform hover:scale-105 transition-transform duration-300 ease-in-out"
